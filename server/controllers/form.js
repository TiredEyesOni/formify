...
export const createForm = (req, res) => {
  ...
  jwt.verify(token, privateKey, async (err, decode) => {
    if (err) {
      return res.statues(401).json({ error: "Token invalid" });
    }
    ...
    try {
      ...
      res.status(201).json({ error: "User doesn't exist with that userId" });
    }
  });
};
...
export const getForms = (req, res) => {
  ...
  jwt.verify(token, privateKey, async (err, decode) => {
    ...
    try {
      ...
      res.status(401).json({ error: "User doesn't exist with that userId" });
    }
  });
};
...
export const handleSubmission = async (req, res) => {
  ...
  try {
    ...
    let keys = Object.keys(submission).filter((value) => {
      value !== "$__" &&
        value !== "$isNew" &&
        value !== "_doc" &&
        value !== "$errors";
    ...
    if (webhookUrl) {
      await axios.post(webhookUrl, {
        ...
        embeds: [
          {
            title: `Hey ${user.fullName} new submission received on register form`,
            color: 1953294,
            fields: fields,
            footer: {
              text: `${new Date().getFullYear()}, formify`,
            },
            timestamp: new Date(),
          },
        ],
  ...
  <html lang="en">
    ...
    return res
      .status(200)
      .json({ message: "we have received your submission" });
  } catch (err) {
    console.error(err);
...
export const getSubmissions = async (req, res) => {
  ...
  if (!token) {
    return res.status(400).json({ error: "tken invalid" });
  }

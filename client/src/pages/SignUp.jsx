...
import darkImg from "../assets/signup2.svg";
import lightImg from "../assets/signup1.svg";
...
const SignUp = ({ isDark }) => {
  ...
  const checkStrength = () => {
    if (strongPassword.test(password)) setStrength("Strong");
    else if (mediumPassword.test(password)) setStrength("Medium");
  ...
  const handleSubmit = async (e) => {
    ...
    if (result.isSuccessful()) setCurrentUser(result.data);
